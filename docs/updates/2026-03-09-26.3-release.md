...
MCP and proxy recording now share the same recorder and produce identical Playwright-compatible trace output.
...
- **Screenshot timing**: MCP recordings now align screenshot timestamps with the action's `endTime` (matching the proxy path) instead of using the later capture time. This fixes timing gaps in the trace viewer.
- **Missing event subscriptions**: MCP recording now subscribes to `log.entryAdded`, `browsingContext.userPromptOpened`, `browsingContext.downloadWillBegin`, `browsingContext.load`, and `browsingContext.fragmentNavigated` — matching the proxy's event coverage.
