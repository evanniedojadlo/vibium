...
V1 shipped in December. Since then: hundreds of commits, a ton of bug fixes, and a completely new feature set. This is the biggest update since launch.

The headline: **168 commands across 23 categories, shipped across 6 implementation targets** — JS async, JS sync, Python async, Python sync, MCP, and CLI. Every target is at parity. The same API surface everywhere.
...
Vibium now uses calendar versioning. `26.2` = February 2026. No more semver, no more "v2" — just the date it shipped. It's been a long run getting to this point. 😏
...
168 commands. 6 targets. All at parity.
...
82 commands across 17 categories. Not a debugging tool — a full automation interface.
...
Working on network interception improvements, video recording, and more AI-native features (`page.do()`, `page.check()`). See the [roadmap](../../ROADMAP.md) for the full picture.
