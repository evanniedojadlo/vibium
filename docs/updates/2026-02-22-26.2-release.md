...
83 commands across 18 categories. not a debugging tool — a full automation interface.

**navigation** — `go`, `back`, `forward`, `reload`, `url`, `title`

**discovery** — `map`, `diff-map`, `find`, `find-all`, `find-by-role`, `a11y-tree`
...
- `browser_find_by_role` — semantic element finding
...
74 `browser_*` tools + 8 `page_clock_*` tools. the full list maps 1:1 to the CLI commands.
