...
The headline: **168 commands across 23 categories, shipped across 6 implementation targets** — JS async, JS sync, Python async, Python sync, MCP, and CLI. Every target is near-complete parity. The same API surface everywhere.
...
On the roadmap: more browser support, video recording, AI-powered locators (`page.do()`, `page.check()`), and Java/.NET clients. See the [roadmap](../../ROADMAP.md) for the full picture — we're building what users ask for.
