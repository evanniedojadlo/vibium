...
- `page.clock.install(opts?)` — JS: override `Date`, `setTimeout`, `setInterval`, `requestAnimationFrame`, `performance.now`. Options: `time` (initial epoch ms), `timezone` (IANA ID via BiDi `emulation.setTimezoneOverride`). Registered as preload script to survive navigations.
- `page.clock.fastForward(ms)` — JS: jump forward, fire each due timer at most once
- `page.clock.runFor(ms)` — JS: advance time, fire all callbacks systematically (reschedules intervals)
- `page.clock.pauseAt(time)` — JS: jump to time and pause
- `page.clock.resume()` — JS: resume real-time progression from current fake time
- `page.clock.setFixedTime(t)` — JS: freeze `Date.now()` permanently at value (timers still run)
- `page.clock.setSystemTime(t)` — JS: set `Date.now()` without triggering timers
- `page.clock.setTimezone(tz)` — BiDi: `emulation.setTimezoneOverride` (IANA ID, empty string to reset)

#### Milestone 7.8: Tracing in Go engine

- `context.tracing.start(opts)` — BiDi: record events + periodic screenshots. Options: name, screenshots, snapshots, sources, title. Produces Playwright-compatible trace zip.
- `context.tracing.stop(opts)` — Package trace into zip. Option: path.
- `context.tracing.startChunk(opts)` — Reset event buffer, increment chunk index. Options: name, title.
- `context.tracing.stopChunk(opts)` — Package current chunk into zip. Option: path.
- `context.tracing.startGroup(name)` — Add group-start marker to trace (Playwright: `tracing.group()`).
- `context.tracing.stopGroup()` — Add group-end marker to trace (Playwright: `tracing.groupEnd()`).
...
// Clock (all 8 methods implemented + timezone via BiDi)
await vibe.clock.install({ timezone: 'Asia/Tokyo' })
await vibe.clock.setFixedTime(new Date('2025-01-01T00:00:00Z'))
const now = await vibe.eval('new Date().getFullYear()')
console.log(now)                                  // → 2025
const hour = await vibe.eval('new Date().getHours()')
console.log(hour)                                 // → 9 (Tokyo = UTC+9)
...
The full engine + JS async API is complete. Before fanning out to other targets, manually verify:
...
|-------|-------|-----------|
| 1 | Navigation + Pages/Contexts | Engine nav + JS async wiring |
| 2 | Element Finding + Chaining | Dual-signature find() |
| 3 | Element Interaction | Auto-wait click/fill/type |
| — | ⚠️ Human Review #1 | Core automation loop works |
| 4 | Element State + Waiting | State queries + wait primitives |
| 5 | Keyboard/Mouse + Screenshots + Eval | Input + capture + eval |
| — | ⚠️ Human Review #2 | Tier 1 + Tier 2 complete |
| 6 | Network + Dialogs | BiDi network interception |
| 7 | Cookies + Frames + Emulation + Rest | Full engine complete |
| — | ⚠️ Human Review #3 | All engine + JS async commands |
| 8 | JS sync wrapper | JS sync mirrors async |
| 9 | Python sync + async | Both Python clients ship |
| — | ⚠️ Human Review #4 | 4 of 6 targets verified |
| 10 | MCP + CLI + AI-Native | All 6 targets + AI methods |
