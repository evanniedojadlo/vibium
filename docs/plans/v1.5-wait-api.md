...
**"Capture"** — subscribe to an event, do the thing that causes it, get the result.
...
### `page.capture.*` — before the action

JS:
```js
// callback style (primary)
const res = await page.capture.response("**/api/data", async () => {
  await el.click();
});
console.log(res.status);

// promise style (escape hatch)
const resPromise = page.capture.response("**/api/data");
await el.click();
...
async with page.capture.response("**/api/data") as res:
    await el.click()
print(res.value)
```
...
|-----------------------|-----------------|
| `capture.response(pat, fn?)` | Response  |
| `capture.request(pat, fn?)`  | Request   |
| `capture.navigation(fn?)`    | URL       |
| `capture.event(name, fn?)`   | Event data|
| `capture.download(fn?)`      | Download  |
| `capture.dialog(fn?)`        | Dialog    |
...
| Old                         | New                                      |
|-----------------------------|------------------------------------------|
| `page.waitForResponse(pat)` | `page.capture.response(pat, fn)`          |
| `page.waitForRequest(pat)`  | `page.capture.request(pat, fn)`           |
| `page.waitForEvent(name)`   | `page.capture.event(name, fn)`            |
...
const [url, res] = await Promise.all([
  ...
  page.capture.response("**/api/data"),
  el.click(),
]);
```

```python
async with page.capture.navigation() as nav:
    async with page.capture.response("**/api/data") as res:
        await el.click()
```

**Event already happened:** `capture.*` will timeout — by design, surfaces ordering bugs. `waitUntil.*` checks current state first and resolves immediately if already true.
...
`using`/`await using` fits lifecycle cleanup (browser, session). Don't use it for `capture` — the value goes out of scope with the block.
